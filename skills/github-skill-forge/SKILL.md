...
description: GitHub repository to AI skill conversion workflow. Use when the user wants to turn a GitHub repository into a reusable AI agent skill, scaffold skill files, aggregate repository context, inspect README/dependencies, or create a standard skill package from an open-source tool. 中文：一个“制造技能的技能”，用于将 GitHub 仓库转换为标准化技能包。
---
...
中文说明：这个 skill 原本面向 Trae 的 `.trae/skills` 工作流；在 Codex 中使用时，重点参考它的“仓库分析、上下文聚合、生成 skill 草稿”流程。路径示例里的 `.trae/skills` 需要按实际环境改成 `~/.codex/skills`、项目内 `.agents/skills`，或你的个人 skills 仓库目录。

## Codex 使用注意

- 保留原始脚本逻辑，但运行命令时优先显式传入 `--output`，避免生成到默认的 Trae 路径。
- 如果目标是个人 Codex skill 仓库，建议输出到 `/Users/zhukaijian/Desktop/Codex/Skills/skills/<skill-name>`。
- 生成草稿后仍需人工或 Codex 二次整理：保留 `name` / `description` 触发字段、补充必要用法、删除无关大文件，并运行 skill 校验。
- 如果只是安装已有标准 Codex skill，优先使用 `$skill-installer`；这个 forge 更适合把“普通 GitHub 工具仓库”整理成新 skill。
