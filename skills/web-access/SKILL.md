...
中文说明：这是一个高权限联网与浏览器自动化 skill，适合需要实时信息、动态页面、登录态页面、本地浏览器历史/书签检索、网页交互或事实核实时使用。它会通过本地 CDP Proxy 操作用户日常浏览器，因此默认只在任务确实需要真实浏览器能力时启用，并避免对敏感账户、支付页面、隐私页面或高频批量操作使用。

来源：<https://github.com/eze-is/web-access>

## Codex 使用边界

- 仅在需要联网查证、动态网页、登录态页面、浏览器历史/书签、截图、媒体帧或网页交互时使用。
- 执行 CDP 操作前，先明确任务目标和成功标准；不为了“更完整”而打开大量页面或反复刷新。
- 默认创建后台 tab 完成任务，结束时关闭自己创建的 tab；不要主动操作或关闭用户原有 tab。
- 使用 `/eval`、`/setFiles`、`/screenshot` 前先确认必要性，避免读取或上传与任务无关的页面内容或本地文件。
- 涉及社交平台、账号后台、风控严格站点时，先提醒用户可能存在限流、封禁或账号风险。
